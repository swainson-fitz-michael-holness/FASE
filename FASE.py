...
def run_fase_given_split(Xtr, ytr, Xva, yva, seed=1337, name="", Sigma_tr=None, Sigma_va=None):
    ...
    print("✅ Final R²=%.4f | GLS-R²=%.4f | MSE=%8.4f | atoms=%s | kept=%s" % (
        val_r2, val_r2_gls, val_mse, str(s1["atoms"][:6]) + ("..." if len(s1["atoms"])>6 else ""), kept))
    model = FASEModel(stage1_applies=s1["apply_fns"], stage2_blocks=accepted_blocks, w=w, b0=b0)
    return dict(R2=val_r2, R2_gls=val_r2_gls, MSE=val_mse, atoms=s1["atoms"], kept=kept, blocks=accepted_blocks,
